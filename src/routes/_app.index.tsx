...
import { articles, caseStudies, sectionOrder } from "@/lib/dummy-data";
...
export const Route = createFileRoute("/_app/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "A calm, bilingual home for family knowledge, stories, and counseling.",
      },
    ],
  }),
  component: HomePage,
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          {sectionOrder.map((key, i) => (
            ...
            >
              ...
              <span className="relative mt-6 text-sm font-semibold">
                {t.knowledge.sections[key]}
              </span>
            </Link>
          ))}
        </div>
      ...
      <Section title={t.home.pdfs}>
        <div className="grid gap-3 sm:grid-cols-2">
          {pdfs.map((p) => (
            <div
              key={p.id}
              className="flex items-center gap-3 rounded-2xl border border-border/60 bg-card p-3 shadow-sm transition-all hover:shadow-soft"
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="truncate text-xs text-muted-foreground">
                  PDF · {p.minutes} {t.common.minutes}
                </p>
              </div>
